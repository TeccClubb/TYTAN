        ...
        coroutineScope.launch {
            try {
                ...
                if (!isServiceRunning) {
                    ...
                    // Only disconnect if we're currently bound
                ...
                // If we're already bound and connected properly, no need to reconnect
                if (connection.isBound && connection.binder != null) {
                    android.util.Log.e("FlutterSingboxPlugin", "Already connected to service, current status: ${_vpnStatus.value}")
                    
                    // Use current status without reconnecting
                    sendStatusUpdate(_vpnStatus.value)
                    return@launch
                }
                
                // Only try to connect if service is running but we're not connected
                if (isServiceRunning && !connection.isBound) {
                    android.util.Log.e("FlutterSingboxPlugin", "Service running but not connected, connecting now")
